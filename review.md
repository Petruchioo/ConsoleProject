...
- `UserService`. Я не уверен, насколько нам нужна проверка существования пользователя при попытке вернуть заметки. Если пользователя нет, можно вернуть пустой список. И учитывая, специфику приложения, мы вряд ли попадем в метод
`GetAllNotes` с несуществующим пользователем. //не понял

- Надо бы согласовать исключения. В `DeleteNote` и `CompleteNote` если заметка не найдена идет кастомное `ArgumentNotFoundExceptions`, в части других методов в этом же случае идет `KeyNotFoundException`. Скорее всего нужно сделать кастомное исключение `NoteNotFoundException` и бросать его. //так и так вроде кастомное или я что то не понял

- `ShowNote` кажется лишним методом. Скорее `Service` должен вызывать `GetNoteById`/`GetAllNotes` и отображать заметку/и.//реализацию в Service => GetAllNotes
...
- `_currentUser` не используется, можно удалить //убрал

- `Service` не должен заниматься валидацией для `UserService`. //исправил

- Лучше сделать кастомное исключение `UserNotFoundException` и бросать его. //сделал
...
- `ShowUser` нужно убрать и заменить примерно так же, как `ShowNote`. //закомитил метод тк в принципе не реализовал вывод пользователей на консоль мб потом доделаю, но раз пока по заданию не надо не буду париться

## Service

- Должен использовать `INote` и `IUser`, получая через конструктор объекты классов, их реализующие. //поправил

- Поля с сервисами для заметок и пользоваетелей не должны быть `static`, как и словарь с методами для меню. Тут можно глянуть подсказку в конце дока. //поправил

- `Console.WriteLine("\nEnter team number");` - что-то явно лишнее или должно быть `command`.//поправил
...
- `Menu` стоит переименовать в `RunApplication` или `PrintMenu`, чтобы был глагол в названии. //поправил

- `try-catch` в методах обработки команд не нужен, так как внутри `Menu` они вызываются уже в `try-catch`.// поправил

- `Console.Clear()` можно унести в `Menu` перед вызовом `action.Invoke()`, чтобы не дублировать во всех методах обработки команд.

## Program

- Нужно создавать тут все экземпляры `NoteService`, `UserService` и передавать их в конструктор `Service`.//исправил
